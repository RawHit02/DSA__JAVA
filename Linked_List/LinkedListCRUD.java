...
public class LinkedListCRUD<T> {
    ...
    int getCount() { // This will count the number of nodes in the LL
        NodeA<T> temp = start;
        int counter = 0;
        while(temp != null) {
            temp = temp.next;
    ...
    void deleteAtEnd(){
        if(start==null){
            System.out.println("Linked list is very empty");
            return;
        }
        if(start.next==null){
            start=null;
    ...
    void reverseLL_recursion(){
        ...
        start = reverseLLrecursion(start,null);
    }
    private NodeA<T> reverseLLrecursion(NodeA<T> current, NodeA<T> prev){
        // Base Case
        if(current== null){
            return prev;
        }
        ...
        current = next;
        return reverseLLrecursion(next,current);
    }
    ...
    public static void main(String[] args) {
        ...
        obj.reverseLL_recursion();
        System.out.println("recursion method to reverse");
        obj.print();
    }
}